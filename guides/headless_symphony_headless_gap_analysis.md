...
| `POST /api/v1/refresh` best-effort trigger | `mix extravaganza.headless.refresh --json` | `POST /api/v1/refresh` (`HeadlessSurface.request_refresh/2`) | no script; no fixture example file | Done |
...
| Refresh/retry/controls as accepted command envelopes | `mix extravaganza.headless.refresh --json`, `mix extravaganza.headless.control --action ... --json` | `POST /api/v1/refresh`, `/subjects/:subject_id/actions/:action`, `/subjects/:subject_id/control/:action` | no direct script for control; review_decision/restart example exists | Done |
| Review listing and decisioning | `mix extravaganza.headless.reviews --json`, `mix extravaganza.headless.review --decision ... --json` | `GET /api/v1/reviews`, `POST /api/v1/reviews/:decision_id/decisions/:decision` | `scripts/headless/review_decision.exs`, fixture `review` output not stored | Done |
...
| Governed source publication | `mix extravaganza.headless.source_publish --json` | `POST /api/v1/source-publication`, `POST /api/v1/subjects/:subject_id/source-publication` | `scripts/headless/source_publish.exs` | Done |
| Source sync/inbound shaping | `mix extravaganza.headless.source.sync --json` | `ProductHost.sync_linear_source/2`, `HeadlessSurface.sync_linear_source/2` | `scripts/headless/source_sync.exs` | Done |
...
| Symphony workflow profile validate/reload | `mix extravaganza.headless.profile_validate --workflow WORKFLOW.md --json`, `mix extravaganza.headless.profile_reload --workflow WORKFLOW.md --json` | `POST /api/v1/profile/validate`, `POST /api/v1/profile/reload`, `GET /api/v1/profile` | `scripts/headless/profile_validate.exs`, `scripts/headless/profile_reload.exs` | Done |
...
| Start run command | `mix extravaganza.headless.start --json` (fixture and live forms) | `ProductHost.start_run/2` + `ExtravaganzaWeb` browser route surface uses same presenters | `scripts/headless/assert_non_fixture_start.exs`, `scripts/headless/start_fixture_run.exs` | Done |
