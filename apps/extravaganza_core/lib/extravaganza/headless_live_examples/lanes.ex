defmodule Extravaganza.HeadlessLiveExamples.Lanes do
  @moduledoc false
  ...
  alias Extravaganza.HeadlessLiveExamples.{Credentials, Envelope}
  ...
  def effect(kind, example, proof, opts) do
    kind
    |> provider_effect_for(example, proof, opts)
    |> Envelope.annotate_provider_effect(opts)
  ...
  defp provider_effect_skipped?(kind, opts) do
    not Credentials.supplied?(kind, opts) or
      (not Credentials.live_product_path?(opts) and Credentials.supplied?(kind, opts))
  end
  ...
  defp skipped_effect(kind, example, opts) do
    %{
      ...
      "status" => "skipped",
      "skip_reason" => Credentials.skip_reason(kind, example, opts),
      "credential_preflight" => Credentials.preflight(kind, example, opts)
    }
  end

  def linear_source_effect(example, proof, opts) do
  ...
  def linear_current_states_effect(example, proof, opts) do
  ...
  def linear_publication_effect(example, proof, opts) do
  ...
  def linear_graphql_tool_effect(example, proof, opts) do
  ...
  def deterministic_memory_tracker_matrix(opts) do
  ...
  def codex_turn_effect(example, _proof, opts) do
  ...
  def github_evidence_effect(example, _proof, opts) do
  ...
  def github_pr_cleanup_effect(example, _proof, opts) do
  ...
  def value(%_{} = struct, key), do: struct |> Map.from_struct() |> value(key)

  def value(%{} = map, key) when is_atom(key) do
  ...
  def value(%{} = map, key) when is_binary(key) do
  ...
  def live_trace_id(opts, operation) do
