# Extravaganza Persistence

## Scope

Extravaganza owns product app using AppKit surfaces and pure Mezzanine.Pack authoring contract. This document is package-local and is the persistence contract for `.` in `extravaganza`.
...
Product source does not own lower stores. Mezzanine repos in config are boot/deployment wiring, not product business-code authority.
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Temporal/Postgres durability lives below AppKit in Mezzanine and is selected by lower owner config and preflight.
...
Product repo owns no lower migrations. Product code must not call lower repos directly.

## Product No-Bypass Scope

Product source scope for no-bypass verification is `apps/**/lib/**/*.ex`. Product business code must use AppKit or product-local public surfaces and must not import lower runtime, store, trace, provider, or governance modules directly.
